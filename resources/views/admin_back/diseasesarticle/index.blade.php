                ...
                <tbody>
                    @foreach ($diseasesarticle as $row)
                        <tr>
							...
							<td>{{ $row->id }}</td>
                            <td>{{ isset($row->diseases->disease_name) ? $row->diseases->disease_name : '' }}</td>
							<td>{{ $row->article_title }}</td>
							<td> <?php echo $row->article_video; ?> </td>
							<td>@if($row->article_profile != '')<img src="{{ asset('public/uploads/thumb') . '/'.  $row->article_profile }}">@endif</td>
							<td>
							{{ str_limit($row->article_description, $limit = 50, $end = '...') }}
